...
class PatternDiscoveryPlugin:
    ...
    def __init__(self, config, ai_service=None, pattern_configs_path: Optional[str] = None):
        ...
        self.pattern_configs: Dict[str, PatternGroupConfig] = self._load_pattern_configurations(pattern_configs_path)

    def _load_pattern_configurations(self, config_path: Optional[str]) -> Dict[str, PatternGroupConfig]:
        """
        ...
        configs["abuse"] = PatternGroupConfig(
            ...
            sub_patterns={
                'escalation_indicators': PatternConfigItem(
                    keywords=['increasingly', 'more frequent', 'getting worse', 'escalating', 'never did this before', 'first time', 'started when', 'progressively worse'],
                ...
                'physical_abuse': PatternConfigItem(
                    keywords=['hit', 'punched', 'kicked', 'slapped', 'choked', 'shoved', 'pushed', 'grabbed', 'restrained', 'assaulted', 'beat', 'bruised', 'injured'],
                ...
                'emotional_verbal_abuse': PatternConfigItem(
                    keywords=['yelled', 'screamed', 'insulted', 'humiliated', 'belittled', 'degraded', 'gaslighting', 'gaslit', 'manipulated', 'threatened', 'intimidated', 'scared', 'afraid', 'worthless', 'stupid', 'crazy', 'unstable', 'name-calling'],
                ...
                'isolation_tactics': PatternConfigItem(
                    keywords=['wouldn\'t let me', 'prevented me from', 'stopped me from seeing', 'blocked me', 'cut off contact with family', 'monitored my calls', 'controlled who I saw'],
                    description_template="Indicates tactics used to isolate the individual.",
                    default_confidence_boost=0.1
                ),
                'financial_abuse_control': PatternConfigItem(
                    keywords=['took my card', 'changed passwords to accounts', 'hid money', 'secret account', 'controlled all spending', 'no access to funds', 'allowance', 'forced me to quit job', 'sabotaged my job', 'ran up debt in my name'],
                    description_template="Indicates financial control as a form of abuse.",
                    default_confidence_boost=0.15
                ),
                'custody_related_threats_coercion': PatternConfigItem(
                    keywords=['take the kids', 'never see them again', 'bad mother', 'bad father', 'unfit parent', 'call CPS', 'get full custody', 'use children against me', 'alienate children'],
                    description_template="Threats or coercion related to child custody.",
                    default_confidence_boost=0.15
                ),
                'technological_abuse': PatternConfigItem(
                    keywords=['spyware', 'stalkerware', 'tracking device', 'GPS tracker', 'monitored my phone', 'hacked my account', 'changed my passwords', 'impersonated me online', 'posted private photos', 'nonconsensual recording', 'cyberstalking', 'doxing'],
        ...
        configs["financial"] = PatternGroupConfig(
            ...
            sub_patterns={
                'hidden_assets_income': PatternConfigItem(
                    keywords=['undisclosed account', 'secret investment', 'offshore', 'shell company', 'cash transactions', 'unreported income', 'deferred compensation', 'crypto wallet', 'missing statements', 'large unexplained withdrawal', 'transfer to unknown'],
                    description_template="Potential indication of hidden assets or income.",
                    default_confidence_boost=0.2
                ),
                 'dissipation_of_assets': PatternConfigItem(
                    keywords=['excessive spending', 'gambling losses', 'gifts to third parties', 'unusual purchases', 'selling assets below market value', 'transferring property to family'],
    ...
    async def discover_patterns_and_theories(
        ...
        timelines: Optional[Dict[str, Any]] = None, # Dict[str, Timeline-like dict]
        image_analyses: Optional[Dict[str, Any]] = None # Dict[str, ImageAnalysisResult-like dict]
        ...
        for file_path, analysis_data in processed_files.items():
            if analysis_data.get('content'):
                all_texts_by_file[file_path].append(("main_content", analysis_data['content']))
            if analysis_data.get('summary'):
                all_texts_by_file[file_path].append(("summary", analysis_data['summary']))
            
            # Handle AI summary (check if it's dict from AIEnhancedLCASPlugin or simple string)
            ...
            if isinstance(ai_summary_data, dict):
                 ...
                 if ai_summary_text: # Ensure it's not an empty string
                    all_texts_by_file[file_path].append(("ai_summary_text", ai_summary_text))
            elif isinstance(ai_summary_data, str) and ai_summary_data:
                 all_texts_by_file[file_path].append(("ai_summary_text", ai_summary_data))
            
            # Handle image analysis text
            if image_analyses and file_path in image_analyses:
                img_analysis_parent = image_analyses[file_path] # This is the value from the image_analyses dict
                # The actual image analysis results might be nested if ImageAnalysisPlugin structures it that way
                ...
                if combined_text:
                    all_texts_by_file[file_path].append(("image_ocr_text", combined_text))
                elif actual_img_analysis_data.get('text_content'): # Fallback if combined_text not present
                    all_texts_by_file[file_path].append(("image_ocr_text", actual_img_analysis_data['text_content']))
            ...
            for group_type, group_config in self.pattern_configs.items():
                for sub_pattern_name, item_config in group_config.sub_patterns.items():
                    ...
                    for keyword in item_config.keywords:
                        ...
                        # Let's assume keywords are meant to be matched as given.
                        try:
                            # Attempt to compile regex for each keyword to catch invalid regex early
                            # More robust: compile all regexes once at init if they are complex
                            compiled_regex = re.compile(r'\b' + re.escape(keyword) + r'\b', re.IGNORECASE)
                        except re.error as e:
                            logger.warning(f"Invalid regex for keyword '{keyword}' in pattern '{sub_pattern_name}': {e}")
                            continue # Skip this keyword

                        for match_obj in compiled_regex.finditer(text_content):
                            snippet = self._get_text_snippet(text_content, match_obj) # Pass match_obj
                            current_matches.append({
                                "keyword": match_obj.group(0), # Get the actual matched text
                                "snippet": snippet,
                            ...
                            matched_keywords_in_subpattern.add(keyword.lower()) # Store the original configured keyword
                    
                    if current_matches:
                        title = f"{group_config.group_type.replace('_', ' ').title()}: {sub_pattern_name.replace('_', ' ').title()}"
                        
                        # Use description_template from config
                        description = item_config.description_template.format(sub_pattern_name=sub_pattern_name.replace('_', ' '))
                        description += f" Identified in '{Path(file_path).name}' based on keywords like '{', '.join(list(matched_keywords_in_subpattern)[:3])}'."
                        
                        # Confidence: base + boost for number of *distinct* keywords from the sub_pattern's list that matched.
                        ...
                        new_pattern = Pattern(
                            ...
                            legal_significance=f"May indicate {group_config.group_type} relevant to {file_analysis_data.get('category', 'various arguments')}.",
                            potential_arguments=[file_analysis_data.get('category', 'General Case File')] if file_analysis_data.get('category') else ['General Case File'],
                            strength_indicators=[f"Distinct keywords matched: {num_distinct_matched_keywords}", f"Total matches: {len(current_matches)}"],
        ...
        try:
            ...
            if response.success:
                try:
                    ...
                    logger.error(f"AI response for pattern analysis was not valid JSON: {response.content}")
                    # Fallback: Try to extract info if it's just plain text
                    return {"ai_description": response.content, "ai_confidence": 0.4, "ai_legal_significance": "AI analysis returned non-JSON content."}
            return None
    ...
    async def _analyze_timeline_for_patterns(self, timeline_name: str, timeline_data: Dict[str, Any]) -> List[Pattern]:
        ...
        for et, count in event_type_counts.items():
            if count >= 3: 
                ...
                try:
                    related_events.sort(key=lambda x: datetime.fromisoformat(x['date'].split(' ')[0].replace('Z', ''))) # Handle ISO dates robustly
                except (ValueError, TypeError, AttributeError) as e:
                    logger.warning(f"Could not sort events by date for timeline pattern analysis due to date format issues: {e}. Events: {related_events[:2]}")
                ...
                pattern = Pattern(
                    ...
                    description=f"{count} events of type '{et}' found in timeline '{timeline_name}'. Average evidence strength of these events: {avg_strength:.2f}.",
                    evidence_files=list(set(f for e_dict in related_events for f_list in [e_dict.get('source_files', []), e_dict.get('supporting_documents', [])] for f in f_list if f)), # Combine and unique
                    confidence_score=min(1.0, 0.6 + count * 0.05 + avg_strength * 0.1), # Factor in avg_strength
                    ...
                    supporting_events=[{'event_title': e.get('title'), 'event_date': e.get('date'), 'description_snippet': e.get('description', '')[:100]} for e in related_events[:5]]
                )
    ...
    def _refine_and_correlate_patterns(self):
        ...
        for p in self.discovered_patterns:
            # Key for merging: type + title (could be more sophisticated, e.g. root keywords)
            ...
            if key not in merged_patterns:
                ...
                existing_p.evidence_files = sorted(list(set(existing_p.evidence_files)))
                ...
                # Recalculate confidence, e.g., average or max, or more complex logic
                existing_p.confidence_score = max(existing_p.confidence_score, p.confidence_score) # Or weighted average
                existing_p.description += f"\nAdditional evidence found in: {', '.join(Path(ef).name for ef in p.evidence_files)}"
                existing_p.strength_indicators.extend(p.strength_indicators)
                existing_p.strength_indicators = sorted(list(set(existing_p.strength_indicators)))


        self.discovered_patterns = list(merged_patterns.values())
        logger.info(f"Refined patterns. Count: {len(self.discovered_patterns)}")
    ...
    async def _synthesize_legal_theories(self):
        ...
        if abuse_pattern_count >= 1 or control_pattern_count >=1: # Lowered threshold for suggestion
            ...
            relevant_patterns_for_abuse_theory = patterns_by_type.get('abuse', []) + patterns_by_type.get('control', [])
            if relevant_patterns_for_abuse_theory:
                supporting_pattern_ids = [p.pattern_id for p in relevant_patterns_for_abuse_theory]
                avg_strength = sum(p.confidence_score for p in relevant_patterns_for_abuse_theory) / len(relevant_patterns_for_abuse_theory)
                ...
                theory = LegalTheory(
                    ...
                    required_evidence_elements=["Specific incidents of abusive/controlling behavior (dates, details)", "Pattern of conduct (not isolated incidents)", "Impact on the victim (fear, financial dependence, etc.)", "Corroborating evidence (texts, emails, photos, witness statements if any)"],
                    ...
                    implementation_steps=["Organize all supporting patterns and their raw evidence chronologically.", "Draft a detailed declaration outlining each incident and the overall pattern of control/abuse.", "Identify which elements of the legal definition of DV/coercive control are met by the evidence.", "Consider if expert testimony (e.g., DV expert) would be beneficial if resources allow.", "Consult legal aid or a domestic violence advocate for guidance on presenting this in court."]
                )
        ...
        if financial_pattern_count >= 1: # Lowered threshold
            relevant_patterns_for_financial_theory = patterns_by_type.get('financial', [])
            if relevant_patterns_for_financial_theory:
                supporting_pattern_ids = [p.pattern_id for p in relevant_patterns_for_financial_theory]
                avg_strength = sum(p.confidence_score for p in relevant_patterns_for_financial_theory) / len(relevant_patterns_for_financial_theory)
                ...
                theory = LegalTheory(
                    ...
                    supporting_patterns=supporting_pattern_ids,
                    evidence_strength=avg_strength,
                    required_evidence_elements=["Proof of undisclosed assets/income (bank statements, tax returns, business records)", "Evidence of asset dissipation (unusual transactions, sales below value)", "Discrepancies in financial declarations vs. actual financials", "Timeline of suspicious financial activities"],
                    ...
                    implementation_steps=["Issue formal discovery requests (subpoenas, interrogatories, requests for production) for all relevant financial records.", "Carefully compare financial declarations with bank statements and other proof.", "Consider if a forensic accountant is necessary (can be expensive, but powerful).", "File motions to compel disclosure or for sanctions if non-compliance occurs.", "Present clear evidence of discrepancies or hidden assets to the court."]
                )
                self._add_theory(theory)
        
        # Abuse of Legal Process / Fraud on the Court
        legal_process_patterns = patterns_by_type.get('legal_process', [])
        # Filter for more specific sub-patterns that indicate serious misconduct
        relevant_fotc_patterns = [
            p for p in legal_process_patterns if 
            any(term in p.title.lower() for term in ['perjury', 'false_statements', 'evidence_tampering', 'procedural_misconduct', 'discovery_abuse'])
        ]
        if relevant_fotc_patterns:
            supporting_pattern_ids = [p.pattern_id for p in relevant_fotc_patterns]
            avg_strength = sum(p.confidence_score for p in relevant_fotc_patterns) / len(relevant_fotc_patterns) if relevant_fotc_patterns else 0.0
            ...
            theory = LegalTheory(
                ...
                supporting_patterns=supporting_pattern_ids,
                evidence_strength=avg_strength,
                required_evidence_elements=["Specific instances of false statements/filings with proof of falsity", "Evidence of intent to deceive the court or harass the other party", "Demonstrable harm or prejudice caused by the misconduct", "Violation of court rules or orders"],
                ...
                implementation_steps=["Meticulously document each instance of misconduct with clear, irrefutable proof (e.g., contradictory documents, proof of lies).", "File appropriate motions with the court (e.g., motion for sanctions, motion to strike, request for evidentiary hearing).", "Clearly highlight contradictions and falsehoods in all court filings and oral arguments.", "Adhere strictly to procedural rules when presenting these claims."]
            )
            self._add_theory(theory)
    ...
    async def _ai_synthesize_theories(self):
        ...
        for p in self.discovered_patterns[:20]: # Limit for token count
            pattern_summary_for_ai.append({
                ...
                "title": p.title,
                "type": p.pattern_type,
                "confidence": f"{p.confidence_score:.2f}", # string for strict json
        ...
        try:
            ...
            if response.success:
                ...
                if not content_to_parse.startswith("[") or not content_to_parse.endswith("]"):
                    logger.error(f"AI theory synthesis response is not a JSON array: {content_to_parse}")
                ...
                try:
                    ...
                    for suggested_theory_data in ai_suggested_theories:
                        if not isinstance(suggested_theory_data, dict): 
                            logger.warning(f"Skipping non-dict item in AI theories: {suggested_theory_data}")
                            continue
                        if 'theory_name' in suggested_theory_data and 'supporting_pattern_ids' in suggested_theory_data:
                            new_theory = LegalTheory(
                                ...
                                description=suggested_theory_data.get('description', ''),
                                supporting_patterns=suggested_theory_data['supporting_pattern_ids'],
                                evidence_strength=float(suggested_theory_data.get('evidence_strength', 0.5)),
                                required_evidence_elements=suggested_theory_data.get('required_evidence_elements', []),
                                strategic_value=suggested_theory_data.get('strategic_value', '')
                            )
                            if not any(t.theory_name.strip().lower() == new_theory.theory_name.strip().lower() for t in self.potential_theories): # Avoid duplicates
                                self._add_theory(new_theory)
                except json.JSONDecodeError as e:
                    logger.error(f"AI response for theory synthesis was not valid JSON after stripping: {e}. Content: {content_to_parse}")
        except Exception as e:
    ...
    def save_discovery_report(self, output_dir_path_str: str):
        ...
        try:
            with open(patterns_file, 'w', encoding='utf-8') as f:
                json.dump([asdict(p) for p in self.discovered_patterns], f, indent=2, ensure_ascii=False)
            logger.info(f"Discovered patterns saved to {patterns_file}")

            with open(theories_file, 'w', encoding='utf-8') as f:
                json.dump([asdict(t) for t in self.potential_theories], f, indent=2, ensure_ascii=False)
            logger.info(f"Potential legal theories saved to {theories_file}")

            with open(summary_report_file, 'w', encoding='utf-8') as f:
                f.write(f"# Pattern Discovery & Legal Theory Summary Report\n\n")
                ...
                f.write(f"## Discovered Patterns Summary ({len(self.discovered_patterns)})\n\n")
                if not self.discovered_patterns:
                    f.write("No significant patterns were automatically discovered based on the current analysis.\n\n")
                else:
                    f.write("| Pattern Title                      | Type              | Confidence | Evidence Files (Count) | Key Significance                       |\n")
                    f.write("|------------------------------------|-------------------|------------|------------------------|----------------------------------------|\n")
                    for p in sorted(self.discovered_patterns, key=lambda x: x.confidence_score, reverse=True)[:20]: # Top 20
                        evidence_count = len(p.evidence_files)
                        f.write(f"| {p.title[:35]}{'...' if len(p.title)>35 else ''} | {p.pattern_type:<17} | {p.confidence_score:<10.2f} | {evidence_count:<22} | {p.legal_significance[:38]}{'...' if len(p.legal_significance)>38 else ''} |\n")
                    if len(self.discovered_patterns) > 20:
                        f.write(f"\n*... and {len(self.discovered_patterns)-20} more patterns. See discovered_patterns_details.json for full list.*\n")
                ...
                f.write(f"## Potential Legal Theories ({len(self.potential_theories)})\n\n")
                if not self.potential_theories:
                    ...
                    f.write("| Theory Name                        | Evidence Strength | Supporting Patterns (Count) | Strategic Value Summary                |\n")
                    f.write("|------------------------------------|-------------------|---------------------------|----------------------------------------|\n")
                    for t in sorted(self.potential_theories, key=lambda x: x.evidence_strength, reverse=True):
                        patterns_count = len(t.supporting_patterns)
                        f.write(f"| {t.theory_name[:35]}{'...' if len(t.theory_name)>35 else ''} | {t.evidence_strength:<17.2f} | {patterns_count:<25} | {t.strategic_value[:38]}{'...' if len(t.strategic_value)>38 else ''} |\n")
            ...
            logger.info(f"Pattern discovery summary report saved to {summary_report_file}")

        except Exception as e:
            logger.error(f"Error saving pattern discovery report: {e}", exc_info=True)