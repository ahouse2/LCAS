...
class ContentExtractionPlugin:
    ...
    def _import_optional_libraries(self):
        ...
        try:
            ...
            logger.warning("pdfplumber not available - advanced PDF extraction disabled")
        ...
        try:
            ...
            logger.warning("email library not available - email extraction disabled")
    
    def extract_content(self, file_path: Path, file_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if extension in self.supported_types:
                ...
                if content:
                    file_analysis['summary'] = self._generate_basic_summary(content)
                ...
                logger.warning(f"Unsupported file type for content extraction: {extension}")
    ...
    def _extract_excel(self, file_path: Path) -> tuple[str, Dict[str, Any]]:
        ...
        try:
            # Read all sheets
            excel_data = self.libraries['pandas'].read_excel(file_path, sheet_name=None)
    ...
    def _extract_csv(self, file_path: Path) -> tuple[str, Dict[str, Any]]:
        ...
        if 'pandas' in self.libraries:
            try:
                ...
                logger.warning(f"Pandas CSV extraction failed for {file_path}: {e}")
...
{body}"""
    ...
    def _generate_basic_summary(self, content: str, max_sentences: int = 3) -> str:
