...
class EvidenceCategorizationPlugin(AnalysisPlugin, UIPlugin):
    ...
    async def analyze(self, data: Any) -> Dict[str, Any]:
            ...
            if file_path.is_file():
                ...
                for folder_name, keywords in self.folder_structure.items():
                    if any(keyword.lower() in filename_lower for keyword in keywords):
                        ...
                        while target_file.exists():
                            ...
                            target_file = target_dir / folder_name / f"{stem}_{counter}{suffix}"
                            counter += 1
                ...
                if not categorized:
                    ...
                    while target_file.exists():
                        stem = file_path.stem
                        suffix = file_path.suffix
                        target_file = review_folder / f"{stem}_{counter}{suffix}"
                        counter += 1
        ...
        total_files = sum(len(files) for files in categorized_files.values()) + len(uncategorized_files)
            ...
            async def run_and_update():
                ...
                def update_ui():
                    if "error" in result:
                        ...
                        self.status_label.config(text=f"Categorized {categorized}/{total} files")
