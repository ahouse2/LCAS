...
from lcas_core import ExportPlugin, UIPlugin
...
class ReportGenerationPlugin(ExportPlugin, UIPlugin):
    ...
    async def export(self, data: Any, output_path: str) -> bool:
        ...
        try:
            ...
            all_results = self.core.analysis_results if hasattr(self.core, 'analysis_results') else {}
            
            # Generate report content
            report_content = self._generate_comprehensive_report(all_results, data)
    ...
    def _generate_comprehensive_report(self, results: Dict[str, Any], data: Dict[str, Any]) -> str:
        ...
        report += f"Source Directory: {data.get('source_directory', 'Unknown')}\n"
        report += f"Target Directory: {data.get('target_directory', 'Unknown')}\n\n"
        ...
        for plugin_name, result_data in results.items():
            ...
            if plugin_name == "File Ingestion":
                report += f"Files Processed: {result.get('files_processed', 0)}\n"
                report += f"Files Copied: {result.get('files_copied', 0)}\n"
                report += f"Backup Directory: {result.get('backup_directory', 'N/A')}\n"
                
            elif plugin_name == "Hash Generation":
                report += f"Files Hashed: {result.get('files_processed', 0)}\n"
                report += f"Hash Report: {result.get('hash_report_path', 'N/A')}\n"
                ...
                report += f"Timeline Report: {result.get('timeline_report_path', 'N/A')}\n"
                
            elif plugin_name == "AI Integration":
                report += f"Files Analyzed: {result.get('files_processed', 0)}\n"
        ...
        if categorization_result:
            categorized_files = categorization_result.get('categorized_files', {})
            for folder, files in categorized_files.items():
                report += f"{folder}: {len(files)} files\n"
            
            uncategorized = categorization_result.get('uncategorized_files', [])
        ...
        if hash_result:
            ...
            report += f"Integrity report: {hash_result.get('integrity_report_path', 'N/A')}\n"
        ...
        if ai_result:
            ...
            report += f"Documents analyzed: {ai_result.get('files_processed', 0)}\n"
        ...
        report += f"Processing completed: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
    ...
    def show_quick_summary(self):
        ...
        if not results:
            messagebox.showinfo("No Results", "No analysis results available. Run some analysis plugins first.")
            return
    ...
    def export_data_ui(self):
        ...
        if not results:
            messagebox.showinfo("No Data", "No analysis data available to export.")
            return
