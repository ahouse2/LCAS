...
class ImageAnalysisPlugin:
    ...
    def _setup_dependencies(self):
        ...
        try:
            ...
            logger.warning(
                "opencv not available - advanced image processing disabled")
        ...
        try:
            ...
            logger.warning(
                "PyMuPDF not available - PDF image extraction disabled")

    async def analyze_file_images(
            self, file_path: Path, file_analysis: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _extract_images_from_file(
            self, file_path: Path) -> List[Tuple[bytes, Dict[str, Any]]]:
        ...
        try:
            ...
            with zipfile.ZipFile(file_path, 'r') as docx:
                ...
                for media_file in media_files:
                    if any(media_file.lower().endswith(ext)
    ...
    async def _analyze_single_image(
            self, image_data: bytes, image_name: str, image_info: Dict[str, Any]) -> ImageAnalysisResult:
        ...
        try:
            ...
            abuse_indicators = self._detect_abuse_patterns(
                text_content, visual_analysis)

            # Financial evidence detection
            financial_evidence = self._detect_financial_evidence(
                text_content, visual_analysis)

            # Communication evidence detection
            communication_evidence = self._detect_communication_evidence(
                text_content, visual_analysis)
    ...
    def _extract_text_from_image(self, image: Image.Image) -> str:
        ...
        try:
            ...
            text = self.libraries['pytesseract'].image_to_string(
    ...
    async def _ai_visual_analysis(
            self, image_data: bytes, text_content: str) -> Dict[str, Any]:
...
"""
    ...
    def _detect_abuse_patterns(
            self, text_content: str, visual_analysis: Dict[str, Any]) -> List[str]:
        ...
        for keyword in abuse_keywords:
            if keyword in text_lower:
                abuse_indicators.append(
                    f"Contains abuse-related language: '{keyword}'")

        # Check for financial control patterns
        for keyword in financial_abuse_keywords:
            if keyword in text_lower:
                abuse_indicators.append(
                    f"Financial control indicator: '{keyword}'")
        ...
        if 'surveillance' in ai_description or 'monitoring' in ai_description:
            abuse_indicators.append(
                "AI detected surveillance/monitoring evidence")

        return abuse_indicators

    def _detect_financial_evidence(
            self, text_content: str, visual_analysis: Dict[str, Any]) -> List[str]:
        ...
        for keyword in financial_keywords:
            if keyword in text_lower:
                financial_evidence.append(
                    f"Financial keyword detected: '{keyword}'")
        ...
        if dollar_matches:
            financial_evidence.append(
                f"Dollar amounts found: {', '.join(dollar_matches[:5])}")
    ...
    def _detect_communication_evidence(
            self, text_content: str, visual_analysis: Dict[str, Any]) -> List[str]:
        ...
        for keyword in messaging_keywords:
            if keyword in text_lower:
                communication_evidence.append(
                    f"Communication platform: {keyword}")
        ...
        ai_description = visual_analysis.get('description', '').lower()
        if 'screenshot' in ai_description and (
                'message' in ai_description or 'chat' in ai_description):
            communication_evidence.append("AI detected message screenshot")

        return communication_evidence

    def _extract_timestamps(self, text_content: str,
    ...
    def _aggregate_image_analysis(
            self, image_analyses: List[ImageAnalysisResult]) -> Dict[str, Any]:
        ...
        all_text = "\n".join(
            [analysis.text_content for analysis in image_analyses])
    ...
    def _update_categorization_from_images(
        ...
        if image_data.get('abuse_indicators'):
            if 'FRAUD_ON_THE_COURT' in str(file_analysis.get('category', '')):
                file_analysis['probative_value'] = min(
                    1.0, file_analysis.get('probative_value', 0) + 0.3)
            if 'ELECTRONIC_ABUSE' in str(file_analysis.get('category', '')):
                file_analysis['probative_value'] = min(
                    1.0, file_analysis.get('probative_value', 0) + 0.2)

        if image_data.get('financial_evidence'):
            if 'NON_DISCLOSURE' in str(file_analysis.get('category', '')):
                file_analysis['probative_value'] = min(
                    1.0, file_analysis.get('probative_value', 0) + 0.4)
