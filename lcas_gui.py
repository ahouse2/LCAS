...
class AIIntegrationPanel(ctk.CTkFrame):
    ...
    def setup_ui(self):
        ...
        self.ai_enabled_var = tk.BooleanVar(
            value=self.config.ai_config.enabled)
        ...
        self.api_key_entry = ctk.CTkEntry(
            self, show="*", placeholder_text="Enter API key...")
        ...
        self.base_url_entry = ctk.CTkEntry(
            self, placeholder_text="https://api.openai.com/v1")
        ...
        self.temperature_var = tk.DoubleVar(
            value=self.config.ai_config.temperature)
    ...
    def test_ai_connection(self):
        ...
        messagebox.showinfo(
            "Test Result",
            "AI connection test would be implemented here!")
...
class LCASMainWindow(ctk.CTk):
    ...
    def create_settings_panel(self):
        ...
        self.probative_weight_var = tk.DoubleVar(
            value=self.config.probative_weight)
        ...
        self.relevance_weight_var = tk.DoubleVar(
            value=self.config.relevance_weight)
        ...
        self.admissibility_weight_var = tk.DoubleVar(
            value=self.config.admissibility_weight)
        ...
        admissibility_slider.grid(
            row=3, column=1, padx=20, pady=5, sticky="ew")
        ...
        neo4j_check = ctk.CTkCheckBox(
            options_frame,
            text="Enable Neo4j Knowledge Graph",
            variable=self.neo4j_var)
    ...
    def create_results_panel(self):
        ...
        # File Analysis tab
        file_analysis_text = ctk.CTkTextbox(
            self.results_notebook.tab("File Analysis"))
        file_analysis_text.pack(fill="both", expand=True, padx=10, pady=10)

        # Argument Strength tab
        argument_text = ctk.CTkTextbox(
            self.results_notebook.tab("Argument Strength"))
        argument_text.pack(fill="both", expand=True, padx=10, pady=10)

        # Duplicates tab
        duplicates_text = ctk.CTkTextbox(
            self.results_notebook.tab("Duplicates"))
    ...
    def run_analysis_thread(self, progress_dialog):
        ...
        try:
            ...
            progress_dialog.update_progress(
                0.1, "Initializing LCAS...", "Setting up analysis engine")
            lcas = LCASCore(lcas_config)

            # Register plugins
            progress_dialog.update_progress(
                0.2, "Loading plugins...", "Registering content extraction plugin")
            lcas.register_plugin(
                'content_extraction',
                ContentExtractionPlugin(lcas_config))

            # Run analysis
            progress_dialog.update_progress(
                0.3, "Starting analysis...", "Beginning file processing")

            # Create folder structure
            progress_dialog.update_progress(
                0.4,
                "Creating folder structure...",
                "Setting up organized directories")
            lcas.create_folder_structure()

            # Discover files
            progress_dialog.update_progress(
                0.5, "Discovering files...", "Scanning source directory")
            files = lcas.discover_files()

            if not files:
                progress_dialog.update_progress(
                    1.0, "Complete", "No files found to process")
                return
            ...
            # Organize files
            progress_dialog.update_progress(
                0.9, "Organizing files...", "Moving files to categorized folders")
            lcas.organize_processed_files()

            # Generate reports
            progress_dialog.update_progress(
                0.95, "Generating reports...", "Creating analysis reports")
            lcas.generate_final_reports()

            # Save results
            progress_dialog.update_progress(
                0.98, "Saving results...", "Saving analysis data")
            lcas.save_analysis_results()

            # Complete
            progress_dialog.update_progress(
                1.0, "Analysis Complete!", "Results saved successfully")

            # Update UI with results
            self.after(
                1000, lambda: self.load_analysis_results(
                    lcas_config.target_directory))

        except Exception as e:
            logger.error(f"Analysis failed: {e}")
            progress_dialog.update_progress(
                0, "Analysis Failed", f"Error: {str(e)}")
    ...
    def load_analysis_results(self, target_directory):
        ...
        try:
            # Load summary report
            summary_file = Path(
                target_directory) / "10_VISUALIZATIONS_AND_REPORTS" / "analysis_summary.md"
            if summary_file.exists():
                with open(summary_file, 'r', encoding='utf-8') as f:
                    summary_content = f.read()

                summary_text = self.results_notebook.tab(
                    "Summary").winfo_children()[0]
                summary_text.delete("1.0", "end")
                summary_text.insert("1.0", summary_content)

            # Load argument strength report
            strength_file = Path(
                target_directory) / "10_VISUALIZATIONS_AND_REPORTS" / "argument_strength_analysis.md"
            if strength_file.exists():
                with open(strength_file, 'r', encoding='utf-8') as f:
                    strength_content = f.read()

                argument_text = self.results_notebook.tab(
                    "Argument Strength").winfo_children()[0]
            ...
            messagebox.showerror(
                "Error",
                f"Error loading analysis results: {
                    str(e)}")
