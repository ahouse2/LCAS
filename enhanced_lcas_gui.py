...
class CaseTheorySetupDialog(ctk.CTkToplevel):
    ...
    def setup_ui(self):
        ...
        self.case_title_entry.grid(row=row, column=1, padx=10, pady=5, sticky="ew")
        ...
        self.case_type_var = ctk.StringVar(value=self.existing_config.get('case_type', 'family_law'))
        self.case_type_combo = ctk.CTkComboBox(
            content_frame,
            values=["family_law", "personal_injury", "business_litigation", "criminal_defense", "employment", "other"],
        ...
        existing_theories = self.existing_config.get('primary_legal_theories', [])
        ...
        save_btn = ctk.CTkButton(button_frame, text="Save & Continue", command=self.save)
    ...
    def load_default_theories(self):
        ...
        theories = default_theories.get(case_type, ["Legal Theory 1", "Legal Theory 2"])
...
Key Facts: {facts_count}"""
        ...
        details_label = ctk.CTkLabel(current_frame, text=details_text, justify="left")
        ...
        if self.config.primary_legal_theories:
            ...
            theories_text = "\n".join(f"• {theory}" for theory in self.config.primary_legal_theories)
            theories_label = ctk.CTkLabel(theories_frame, text=theories_text, justify="left")
    ...
    def create_settings_panel(self):
        ...
        ai_check = ctk.CTkCheckBox(ai_frame, text="Enable AI Analysis", variable=self.ai_enabled_var)
        ai_check.grid(row=1, column=0, columnspan=2, padx=20, pady=5, sticky="w")
        
        # OpenAI API Key
        ctk.CTkLabel(ai_frame, text="OpenAI API Key:").grid(row=2, column=0, padx=20, pady=5, sticky="w")
        self.openai_key_entry = ctk.CTkEntry(ai_frame, placeholder_text="sk-...", show="*")
        self.openai_key_entry.grid(row=2, column=1, padx=20, pady=5, sticky="ew")
        if self.config.openai_api_key:
            self.openai_key_entry.insert(0, self.config.openai_api_key)
        
        # Analysis Depth
        ctk.CTkLabel(ai_frame, text="Analysis Depth:").grid(row=3, column=0, padx=20, pady=5, sticky="w")
        self.analysis_depth_var = ctk.StringVar(value=self.config.analysis_depth)
        ...
        self.viz_var = ctk.BooleanVar(value=self.config.generate_visualizations)
    ...
    def create_results_panel(self):
        ...
        self.results_text.grid(row=1, column=0, padx=20, pady=20, sticky="nsew")
        self.results_text.insert("1.0", "Analysis results will appear here after running the analysis...")
    ...
    def configure_case_theory(self):
        ...
        if dialog.result:
            ...
            messagebox.showinfo("Success", "Case theory configuration updated successfully!")
    ...
    def quick_start_analysis(self):
        ...
        if not self.config.source_directory:
            messagebox.showinfo("Setup Required", "Please set up your source directory first.")
            self.show_panel("files")
            return
        
        if not self.config.target_directory:
            # Set default target directory
            self.config.target_directory = os.path.join(os.path.expanduser("~"), "Desktop", "LCAS_Results")
            self.target_entry.delete(0, "end")
            self.target_entry.insert(0, self.config.target_directory)
            self.save_config()
    ...
    def create_progress_dialog(self):
        ...
        dialog.progress_label.grid(row=0, column=0, padx=20, pady=10, sticky="ew")
        
        # Progress bar
        dialog.progress_bar = ctk.CTkProgressBar(dialog)
        dialog.progress_bar.grid(row=1, column=0, padx=20, pady=10, sticky="ew")
        dialog.progress_bar.set(0)
        
        # Details text
        dialog.details_text = ctk.CTkTextbox(dialog)
        dialog.details_text.grid(row=2, column=0, padx=20, pady=10, sticky="nsew")
    ...
    def run_analysis_thread(self, progress_dialog):
        ...
        try:
            ...
            progress_dialog.progress_label.configure(text="Creating folder structure...")
            progress_dialog.progress_bar.set(0.1)
            progress_dialog.details_text.insert("end", "Setting up organized directories\n")
            ...
            progress_dialog.progress_label.configure(text="Discovering files...")
            progress_dialog.progress_bar.set(0.3)
            progress_dialog.details_text.insert("end", "Scanning source directory for evidence\n")
            progress_dialog.update()
            time.sleep(1)
            ...
            progress_dialog.details_text.insert("end", f"Found {file_count} files for analysis\n")
            progress_dialog.update()
            ...
            for i in range(file_count):
                ...
                progress_dialog.progress_label.configure(text=f"Processing file {i+1}/{file_count}")
                
                if i < 5:  # Show first few files
                    progress_dialog.details_text.insert("end", f"Analyzing file {i+1}\n")
            ...
            progress_dialog.details_text.insert("end", "\nAnalysis completed successfully!\n")
            progress_dialog.details_text.insert("end", f"Processed {file_count} files\n")
            progress_dialog.details_text.insert("end", f"Results saved to: {self.config.target_directory}\n")
            progress_dialog.update()
