...
from typing import Dict, List, Any, Union # Removed unused 'Union' if not needed by later logic
...
class HashGenerationPlugin(AnalysisPlugin, UIPlugin):
    ...
    async def analyze(self, data: Any) -> Dict[str, Any]:
        ...
        if hasattr(self.core, 'config') and hasattr(self.core.config, 'hash_generation_types'):
            # Ensure config value is a list, otherwise use default
            config_hashes = getattr(self.core.config, 'hash_generation_types')
            if isinstance(config_hashes, list) and config_hashes: # check if not empty list
                default_hashes_from_config = config_hashes

        hash_types_to_generate = data.get("hash_types", default_hashes_from_config)
        if isinstance(hash_types_to_generate, str):
        ...
        return {
            ...
            "file_hashes_data": file_hashes_data
        }
    ...
    def view_hash_report(self):
        ...
        last_result_wrapper = self.core.get_analysis_result(self.name) # This returns the wrapper
        json_report_path_str = None
        if last_result_wrapper and isinstance(last_result_wrapper.get("result"), dict): # Check if result is a dict
            json_report_path_str = last_result_wrapper["result"].get("json_report_path")
    ...
    def run_analysis_ui(self):
        ...
        if hasattr(self.core, 'update_status'): self.core.update_status("Hash Generation (UI Trigger) started...")
