...
@cli.command()
@click.option('--config', default='lcas_config.json', help='Configuration file path')
...
def analyze(config, source, target, case_name):
    ...
    if not core.config.source_directory:
        core.config.source_directory = Prompt.ask("Enter source directory path")
    
    if not core.config.target_directory:
        core.config.target_directory = Prompt.ask("Enter target directory path")
...
async def run_analysis_cli(core: LCASCore):
    ...
    try:
        with Progress(
            ...
            # Initialize core
            init_task = progress.add_task("Initializing LCAS Core...", total=None)
            ...
            for plugin in analysis_plugins:
                plugin_task = progress.add_task(f"Running {plugin.name}...", total=None)
...
def status(config):
    ...
    if Path(config).exists():
        ...
        console.print(f"[yellow]⚠ No configuration file found at {config}[/yellow]")
        core = LCASCore(LCASConfig())
    ...
    table.add_row("Source Directory", core.config.source_directory or "Not set")
    table.add_row("Target Directory", core.config.target_directory or "Not set")
    ...
    if core.config.target_directory:
        ...
        if target_path.exists():
            ...
            console.print(f"[yellow]⚠ Target directory will be created[/yellow]")
...
def plugins(config):
    ...
    core = LCASCore.load_config(config) if Path(config).exists() else LCASCore(LCASConfig())
...
def config():
    ...
    if not Path(source_dir).exists():
        if not Confirm.ask(f"Source directory '{source_dir}' does not exist. Continue anyway?"):
            return
